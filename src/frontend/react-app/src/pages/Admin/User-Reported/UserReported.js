...
const UserReported = () => {
  ...
  return (
    <>
      <div className={`col-12 px-4 Medium-Body-sub-text-2 mb-lg-4 ${isDark && "bg-custom-black text-light"}`}>
        ...
        <div className="card-section">
         {
          reportedUsers.map(report => (
            <div className={`card flex-row align-items-center px-3 px-lg-5 border-0  ${isDark && "bg-dark text-light"}`} key={report._id}
              onClick={() => navigate(`/admin-dashboard/reported-users/${report.user._id}`)}
            >
            {
